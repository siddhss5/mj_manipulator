...
class TestTickDrivenExecution:
    ...
    def test_tick_advances_runner(self, loop, controller):
        ...
        np.testing.assert_allclose(controller._arms["test_arm"].target_position, [0.0, 0.0])

        loop.tick()
        np.testing.assert_allclose(controller._arms["test_arm"].target_position, [0.1, 0.1])
...
class TestTickDrivenBimanual:
    ...
    def test_trajectory_and_manual_targets(self, loop, two_arm_controller):
        ...
        np.testing.assert_allclose(ctrl._arms["arm2"].target_position, [0.7, 0.7])
